...
from torch.nn import CrossEntropyLoss, MSELoss, Sigmoid
...
# Below is self-added
@add_start_docstrings(
    """ELECTRA Model transformer with a sequence ranking head on top (a linear layer + a sigmoid func on top of
    the pooled output) e.g. for Evaluation tasks. """,
    ELECTRA_START_DOCSTRING,
)
class ElectraForEvaluation(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = 1
        self.num_rankings = config.num_rankings
        self.electra = ElectraModel(config)
        self.classifier = ElectraClassificationHead(config)
    ...
    @add_code_sample_docstrings(
        tokenizer_class=_TOKENIZER_FOR_DOC,
        checkpoint="google/electra-small-discriminator",
        output_type=SequenceEvaluationOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
        ...
        logits = self.classifier(sequence_output)
        acti_func=Sigmoid()
        scores = acti_func(logits)*self.num_rankings
        ...
        if return_tuple:
            output = (scores,) + discriminator_hidden_states[1:]
            return ((loss,) + output) if loss is not None else output

        return SequenceEvaluationOutput(
            loss=loss,
            scores=scores,
            hidden_states=discriminator_hidden_states.hidden_states,
            attentions=discriminator_hidden_states.attentions,
...
@add_start_docstrings(
    """ELECTRA Model transformer with a sequence ranking head on top (a linear layer + a sigmoid func on top of
    the pooled output) e.g. Evaluation as pretraining tasks. """,
    ELECTRA_START_DOCSTRING,
)
# Take a dialog as its input, use evaluation as its pretraining-method
class ElectraPertrainingForEvaluation(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = 1
        self.num_rankings = 2
    ...
    def forward(
        ...
        sequence_output = discriminator_hidden_states[0]
        logits = self.classifier(sequence_output)
        acti_func=Sigmoid() # 0-1
        scores = acti_func(logits)*2-1 # range from -1 to +1
