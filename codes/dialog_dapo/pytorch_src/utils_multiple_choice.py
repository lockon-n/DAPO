    ...
    def _create_examples(self, lines: List[List[str]], type: str):
        ...
        for (i, e) in enumerate(lines):
            ...
            for (t, qas) in enumerate(qas_s):
                ...
                choices = qas['choice']
                answer = qas['answer']
                label = str(choices.index(answer))
                examples.append(InputExample(example_id=guid_,
                                             question=question,
...
class MutualProcessor(DataProcessor):
    """Processor for the Mutual/Mutual_plus data set."""
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
            ...
            inputs = tokenizer(
                ...
                padding="max_length",
                truncation_strategy='only_first_from_head',
...
MULTIPLE_CHOICE_TASKS_NUM_LABELS = {
    "race", 4, "swag", 4, "arc", 4, "syn", 5, 'dream', 3, "mutual", 4, "mutual_plus", 4}
