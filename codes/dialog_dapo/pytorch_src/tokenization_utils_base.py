...
class TruncationStrategy(ExplicitEnum):
    ...
    ONLY_FIRST_FROM_HEAD = "only_first_from_head"
        ...
        if truncation is False and old_truncation_strategy != "do_not_truncate":
            if verbose:
                warnings.warn(
                    ...
                    "`truncation='longest_first'` (will iteratively remove tokens from the longest sentence in the pairs)."
                    "`truncation='only_first_from_head'` (will iteratively remove tokens from the first sentence in the pairs, from its head).",
    ...
    def truncate_sequences(
        ...
        Args:
            ...
            truncation_strategy (:obj:`string`, `optional`, defaults to "longest_first"):
                ...
                - 'do_not_truncate'
                - 'only_first_from_head' : Only truncate the first sequence, and we truncate it from its head.
            stride (:obj:`int`, `optional`, defaults to ``0``):
        ...
        elif truncation_strategy == TruncationStrategy.ONLY_FIRST_FROM_HEAD:
            if len(ids) > num_tokens_to_remove:
                window_len = min(len(ids), stride + num_tokens_to_remove)
                overflowing_tokens = ids[:window_len]
                ids = ids[num_tokens_to_remove:]
