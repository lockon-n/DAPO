...
from modeling_electra import (
    ...
    ElectraForEvaluation,
    ElectraPertrainingForEvaluation,
    ElectraModel,
)
...
MODEL_FOR_EVALUATION_MAPPING = OrderedDict(
    [
        (ElectraConfig, ElectraForEvaluation),
    ]
)

MODEL_FOR_PRETRAINING_EVALUATION_MAPPING = OrderedDict(
    [
        (ElectraConfig, ElectraPertrainingForEvaluation),
    ]
)
...
# Below is self-added
class AutoModelForEvaluation:
    r"""
        :class:`~transformers.AutoModelForEvaluation` is a generic model class
        that will be instantiated as one of the sequence classification model classes of the library
        when created with the `AutoModelForEvaluation.from_pretrained(pretrained_model_name_or_path)`
        class method.
    ...
    def __init__(self):
        raise EnvironmentError(
            "AutoModelForEvaluation is designed to be instantiated "
            "using the `AutoModelForEvaluation.from_pretrained(pretrained_model_name_or_path)` or "
            "`AutoModelForEvaluation.from_config(config)` methods."
        )
    ...
    def from_config(cls, config):
        ...
        Args:
            config (:class:`~transformers.PretrainedConfig`):
                ...
                - isInstance of `electra` configuration class: :class:`~transformers.FlaubertForEvaluation` (Electra model)


        Examples::

            config = BertConfig.from_pretrained('bert-base-uncased')    # Download configuration from S3 and cache.
            model = AutoModelForEvaluation.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        for config_class, model_class in MODEL_FOR_EVALUATION_MAPPING.items():
            if isinstance(config, config_class):
                return model_class(config)
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_EVALUATION_MAPPING.keys()),
            )
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        falling back to using pattern matching on the `pretrained_model_name_or_path` string:
            ...
            - `electra`: :class:`~transformers.ElectraForEvaluation` (Electra model)
        ...
        Examples::
            ...
            model = AutoModelForEvaluation.from_pretrained('electra-small-discriminator')    # Download model and configuration from S3 and cache.
            model = AutoModelForEvaluation.from_pretrained('./test/electra_model/')  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
            assert model.config.output_attention == True
            # Loading from a TF checkpoint file instead of a PyTorch model (slower)
            config = AutoConfig.from_json_file('./tf_model/bert_tf_model_config.json')
            model = AutoModelForEvaluation.from_pretrained('./tf_model/bert_tf_checkpoint.ckpt.index', from_tf=True, config=config)
...
class AutoModelPretrainingForEvaluation:
    r"""
        :class:`~transformers.AutoModelPretrainingForEvaluation` is a generic model class
        that will be instantiated as one of the sequence classification model classes of the library
        when created with the `AutoModelPretrainingForEvaluation.from_pretrained(pretrained_model_name_or_path)`
        class method.
    ...
    def __init__(self):
        raise EnvironmentError(
            "AutoModelPretrainingForEvaluation is designed to be instantiated "
            "using the `AutoModelPretrainingForEvaluation.from_pretrained(pretrained_model_name_or_path)` or "
            "`AutoModelPretrainingForEvaluation.from_config(config)` methods."
        )
    ...
    def from_config(cls, config):
        ...
        Examples::
            ...
            config = BertConfig.from_pretrained('bert-base-uncased')    # Download configuration from S3 and cache.
            model = AutoModelPretrainingForEvaluation.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        for config_class, model_class in MODEL_FOR_PRETRAINING_EVALUATION_MAPPING.items():
            if isinstance(config, config_class):
                return model_class(config)
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_PRETRAINING_EVALUATION_MAPPING.keys()),
            )
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Examples::
            ...
            model = AutoModelPretrainingForEvaluation.from_pretrained('electra-small-discriminator')    # Download model and configuration from S3 and cache.
            model = AutoModelPretrainingForEvaluation.from_pretrained('./test/electra_model/')  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
            assert model.config.output_attention == True
            # Loading from a TF checkpoint file instead of a PyTorch model (slower)
            config = AutoConfig.from_json_file('./tf_model/bert_tf_model_config.json')
            model = AutoModelPretrainingForEvaluation.from_pretrained('./tf_model/bert_tf_checkpoint.ckpt.index', from_tf=True, config=config)
